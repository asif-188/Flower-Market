...
import { subscribeToCollection, saveProduct, deleteProduct } from '../utils/storage';
...
const Flowers = () => {
    ...
    const handleSave = async (e) => {
        ...
        const isTaDuplicate = taNameLower && flowers.some(f => 
            f.id !== editing?.id && 
            f.taName?.trim().toLowerCase() === taNameLower
        );

        if (isDuplicate) {
            alert(`❌ Failed: A flower with the name "${form.name}" already exists.`);
            return;
        }

        if (isTaDuplicate) {
            alert(`❌ Failed: A flower with the Tamil name "${form.taName}" already exists.`);
            return;
        }
